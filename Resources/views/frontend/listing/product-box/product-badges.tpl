...
{block name='frontend_listing_box_article_discount_instock_badge'}
    {if $theme.product_box_badge_instock != 0 AND !$sArticle.esd}
        ...
        {if $sArticle.attributes.mxc_instock->get('instock') < 1}
        ...
        {elseif $sArticle.attributes.mxc_instock->get('instock') le $theme.product_box_badge_instock_threshold}
            <div class="product--badge badge--low-stock">
                {s name="twBadgeLowStockTextPrefix"}Nur{/s} {$sArticle.attributes.mxc_instock->get('instock')|number} {s name="twBadgeLowStockTextSuffix"}auf Lager!{/s}
            </div>
        {/if}
    {/if}
{/block}
